...
local function BuildGroupPanel(width)
	...
	b:Check("Trackers drawn by the game", function() local g = G() return g and g.gameDrawn or false end,
		...
		"Each tracker in this group gets a game-owned frame filtered to its spell. The game shows it while the aura is on you, in combat too, and hides it when it is gone, uncovering the tracker's 'missing' art. So active and missing both stay right in combat. Every tracker keeps a cell (the layout cannot follow what the game hides), the warn window does not apply, and a tracker the ledger has no spell ID for is drawn by the addon as before.")
